...
<!-- セッションスコープに保存する参考 -->
<!-- https://atmarkit.itmedia.co.jp/ait/articles/0108/29/news002.html -->
<!-- 各スコープについて -->
...
<!-- ここから出退勤休憩入り休憩戻りボタン関連 -->


<!-- 1行目と2行目全体にかかるタグ開始 -->
<!-- オンロードでボタン押下後処理時間に関するメソッド読み出しておく。 -->
<div onload="pushed_Attendance()" class="content buttons">

	<!-- ここで1行目全体にかかるタグ開始 -->
	<!-- ここで1行目の出勤ボタン、退勤ボタン全体にかかるタグを入れる -->
	<ul>

		<!-- ここで1行目の出勤ボタン全体にかかるタグ開始 -->
		<li>
		<form action="maintest.jsp" method="get">
		...
		<input id="attendanceIntimeValueInput" type="hidden" name="intimeValue" value="">
			...
			<!-- 1行目。表示されている出勤ボタン部分（非モーダル)にかかるタグ開始 -->
			<!-- クリックでモーダル1を表示 -->
			<!-- ...やはり、モーダルオープンjsとサブミットはどちらか一つしか実行されない様子 -->
			...
			<!--<button onclick="pushed_Attendance()" class="js-modal-open" type="submit" data-target="modal01">出勤</button><br> -->
				...
				<!-- ここから下で1行目着火するまで非表示のモーダル内表示に関するタグ開始 -->
				<!-- ここから下でモーダル内の表示を実装 -->
				...
				<!-- ここから下で1行目着火するまで非表示のモーダル内表示に関するタグ閉じ -->
				</div>
				</div>

			<!-- 1行目。表示されている出勤ボタン部分（非モーダル)にかかるタグ閉じ -->

		<!-- ここで1行目の出勤ボタン全体にかかるタグ閉じ -->
		</form>
		</li>

	<!-- 1行目全体にかかるタグ閉じ -->
	<!-- ここで1行目の出勤ボタン、退勤ボタン全体にかかるタグを閉じる -->
	</ul>>
<!-- 1行目と2行目全体にかかるタグ閉じ -->
</div>
